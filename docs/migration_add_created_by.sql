-- Migration: Add created_by column to migrations table and update dashboard view
-- Run this on your PostgreSQL database

-- Add the created_by column (may already exist in schema.sql but not in live DB)
ALTER TABLE migrations
ADD COLUMN IF NOT EXISTS created_by UUID REFERENCES team_members(id);

-- Recreate the dashboard view to include created_by, updated_at, and creator name
